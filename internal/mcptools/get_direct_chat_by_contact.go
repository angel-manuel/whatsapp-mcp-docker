...
WHERE c.is_group = 0 AND c.jid = ?`
	err := store.DB().QueryRowContext(ctx, query, input).Scan(
		&jid, &name, &isGroup, &chatType, &ts, &body, &id, &sender, &isFromMe, &hasMessage,
	)
...
LIMIT 1`, "%"+likeQuery+"%").Scan(
	...
	return buildChatDTO(jid, name, isGroup != 0, chatType, ts, hasMessage == 1, body, id, sender, isFromMe != 0), nil
}
